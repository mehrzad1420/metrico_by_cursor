...
- **Erasure:** Company Info → email OTP → Edge Function `secure-delete-account` (preferred) or RPC path after `account-deletion-server.sql`.
- **Export:** Backup v4 merges local state + `export_my_data` RPC when available.
