...
<p align="center">
  <img src="https://img.shields.io/badge/Version-2.52.3-blue?style=for-the-badge" alt="Version"/>
...
- Run SQL scripts in order (see `supabase/README.md`):
  `schema.sql` → `plans.sql` → `admin.sql` → `admin-members-tools.sql` → `owner-portals.sql` → `owner-portal-messages.sql` → `product-feedback.sql` → `demo-readonly.sql` → `plan-enforcement.sql` → `account-deletion.sql` → `profiles-hardening.sql`
...
<p align="center"><sub>Metrico v2.52.3 · Built for the construction industry in Iran</sub></p>
