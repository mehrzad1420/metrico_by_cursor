...
- Run SQL scripts in order (see `supabase/README.md`):
  `schema.sql` → `plans.sql` → `admin.sql` → `admin-members-tools.sql` → `owner-portals.sql` → `owner-portal-messages.sql` → `product-feedback.sql` → `demo-readonly.sql` → `plan-enforcement.sql` → `account-deletion.sql`
...
<p align="center"><sub>Metrico v2.52.1 · Built for the construction industry in Iran</sub></p>
